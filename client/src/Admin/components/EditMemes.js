...
const EditMemes = (props) => {
  const {
    classes, modalState, handleClose, memeInfo,rows,stopFlags
  } = props;
  ...
  async function handleActivate(meme) {
    ...
    await memeNetwork.methods.approveMeme(meme.memeId).send({from : result[0]});
  ...
  async function  handleDeactivate (meme) {
    ...
    await memeNetwork.methods.rejectMeme(meme.memeId).send({from : result[0]});
          ...
          <Card className={classes.paper}>
            <div style={{position:'static', minHeight: '65px'}}>
                ...
                <div style={{ width: "25%", float: "right"}}>
                    {(memeInfo.memeStatus === 'Approved' && memeInfo.memeFlags > stopFlags) && 
                        <Button disabled fullWidth style={{backgroundColor:'#57bb8aff', color:'white'}}>
                            {memeInfo.memeStatus}
                        </Button>
                    }
                    {(memeInfo.memeStatus === 'Pending' || memeInfo.memeStatus === 'Rejected' || memeInfo.memeFlags <= stopFlags) && 
            ...
            {(memeInfo.memeStatus === 'Rejected' || memeInfo.memeStatus === 'Pending') &&
                <div style={{position:'relative', height:'220px', paddingTop:'10px'}}>
                    <div style={{ width: "45%", float: "left"}}>
                        ...
                        />
                        <Button fullWidth style={{backgroundColor:'#5d4037ff', color:'white'}} onClick={() => handleActivate(memeInfo)}>
                            Approve Meme
                        </Button>
                    </div>
                </div>
            }
            {(memeInfo.memeFlags > stopFlags || memeInfo.memeStatus === 'Approved') &&
                <div style={{position:'relative', height:'220px', paddingTop:'10px'}}>
                  <CardMedia
                      component="img"
                      className={classes.media}
                  ...
                  /> 
                  <Button fullWidth style={{backgroundColor:'#ffd966ff'}} onClick={() => handleDeactivate(memeInfo)}>
                      Deactivate Meme
                  </Button>
                </div>
            }
